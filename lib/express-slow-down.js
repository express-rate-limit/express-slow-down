...
async function defaultDelayMsFn(used, req, res) {
  // note: `this` is the options object
  const delayAfter = await getValue(this.delayAfter, this, [req, res]);
  // this function is only called when delayMs is a number, so we shouldn't need to await it
  return (used - delayAfter) * this.delayMs;
}
...
function slowDown(opts = {}) {
  ...
  const options = {
    ...
    delayMs: 1000, // milliseconds - default delays by 1 second per hit after the first hit
    ...
    delayMsFn:
      typeof opts.delayMs === "function" ? opts.delayMs : defaultDelayMsFn,
    ...
    handler: async (req, res, next) => {
      ...
      const unboundedDelay = await getValue(options.delayMsFn, options, [
        used,
        req,
        res,
      ]);
